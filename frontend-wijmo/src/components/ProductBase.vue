    ...
    export default{
        ...
        methods:{
            ...
            async getRealEntity(id){
               try{
                    let result = await axios.get(axios.fixUrl(`/products/${id}`))
                    result.data.id = id;
                    return result.data;
                }catch(e){
                    return null;
            ...
            async search(query) {
                ...
                var temp = null;
                if(query!=null){
                    temp = await axios.put(axios.fixUrl('/products/search/findByProductNoOrProductNameOrProductStandard'), { productNo: query.parameters.productNo, productName: query.parameters.productName, productStandard: query.parameters.productStandard });
                    me.values = await me.processData(temp.data);
                }else{
                    temp = await axios.get(axios.fixUrl('/products'))
                    me.values = await me.processData(temp.data._embedded.products);
                }
                return me.values;
            },
        },
